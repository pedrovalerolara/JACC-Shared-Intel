...
function JACC.parallel_for((M, N)::Tuple{I,I}, f::F, x::Vararg{Union{<:Number,<:ROCArray}}) where {I<:Integer,F<:Function}
  ...
  #@roc groupsize = (Mthreads, Nthreads) gridsize = (Mblocks, Nblocks) shmem = numThreads * numThreads * sizeof(Float32) _parallel_for_amdgpu_MN(f, x...)
...
function JACC.shared(x::ROCDeviceArray{T,N}) where {T,N}
  ...
  shmem = @ROCStaticLocalArray(Float32, 128)
